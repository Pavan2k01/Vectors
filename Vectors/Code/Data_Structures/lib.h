...
void find_intersection(Node* line1, Node* line2, Node* line3) {
    ...
    for (int i = 0; i < numRows; i++) {
        for (int j = 0; j < numCols; j++) {
            if (i == 0 && j == 0) {
            	...
            	coefficients[6] = get(line3, i-2, j);
            	coefficients[7] = get(line3, i-1, j);
            	coefficients[8] = get(line3, i, j);
            }
        }
    }

    double determinant = coefficients[2] * coefficients[3] - coefficients[5] * coefficients[0];

    if (determinant != 0) {
        // Calculate the intersection point (x, y)
        double x = (coefficients[1] * coefficients[3] - coefficients[4] * coefficients[0]) / determinant;
        double y = (coefficients[1] * coefficients[5] - coefficients[4] * coefficients[2]) / determinant;
        double p = determinant;
